...
#you should put master10K.csv etc in this address
Storge_address = "/Volumes/12321/Data10K1"
...
data = pd.read_csv(Storge_address+'/master10K.csv')
data["Label"]= data['CIK'].map(lambda x: "False")
data["File_name"]=data['Name'].map(lambda x: get_filename(x))
data.to_csv(Storge_address+'master10K_Crosscheck.csv')


#when you have 'master10K_Crosscheck.csv' in Storge_address, run code from there to download:
data = pd.read_csv(Storge_address+'master10K_Crosscheck.csv')
